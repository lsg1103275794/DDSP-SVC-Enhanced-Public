...
def get_model_from_config(model_type, config_path):
    ...
    if model_type == "mdx23c":
        ...
        # 过滤掉 MelBandRoformer 不支持的参数
        model_config = dict(config.model)
        unsupported_params = ['skip_connection', 'mlp_expansion_factor', 'use_torch_checkpoint']
            ...
            while i < mix.shape[1]:
                ...
                if len(batch_data) >= batch_size or (i >= mix.shape[1]):
                    ...
                    for j in range(len(batch_locations)):
                        start, seg_len = batch_locations[j]
                        
                        # 使用模型输出的实际长度和 seg_len 中的较小值
                        # 这样可以处理模型边界裁剪的情况
                        actual_len = min(seg_len, x[j].shape[-1])
                        
                        # 使用 actual_len 确保所有切片长度一致
                        result[..., start : start + actual_len] += x[j][..., :actual_len].cpu() * window[..., :actual_len]
